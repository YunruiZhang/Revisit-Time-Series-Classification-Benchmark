...
# notNormalised=["ArrowHead","Beef","BeetleFly","BirdChicken","Coffee","Computers",
#               "Cricket_X","Cricket_Y","Cricket_Z","DistalPhalanxOutlineAgeGroup",
#               "DistalPhalanxOutlineCorrect","DistalPhalanxTW","ECG200","Earthquakes",
#               "ElectricDevices","FordA","FordB","Ham","Herring","LargeKitchenAppliances",
#               "Meat","MiddlePhalanxOutlineAgeGroup","MiddlePhalanxOutlineCorrect","MiddlePhalanxTW",
#               "OliveOil","PhalangesOutlinesCorrect","Plane","ProximalPhalanxOutlineAgeGroup",
#               "ProximalPhalanxOutlineCorrect","ProximalPhalanxTW","RefrigerationDevices",
#                "ScreenType","ShapeletSim","ShapesAll","SmallKitchenAppliances","Strawberry",
#                "ToeSegmentation1","ToeSegmentation2","UWaveGestureLibraryAll","UWaveGestureLibrary_Z",
...
class logger:
    ...
    def log(self, dataset, results):
        ...
        self.df.to_csv(f"cor_nor_seed{sd}.csv")
...
my_log = logger(["mini_rck", "TFC" , "rck_shu", "TFC_shu"], datasets)
...
for a in subfolders:
    ...
    if dataset_name in equalLengthProblems:
        ...
        clf = RocketClassifier(rocket_transform='minirocket', n_jobs=20, random_state=sd)
        clf.fit(train_x, train_y)
        res = clf.predict(test_x)
        cr = classification_report(test_y, res, output_dict = True)
        real_res.append(cr['accuracy'])
        print(f"------------------------rck acc is {cr['accuracy']}--------------------------------------")


        clf = TimeSeriesForestClassifier(n_jobs=1, random_state=sd)
        clf.fit(train_x, train_y)
        y_pred = clf.predict(test_x)
        cr = classification_report(test_y, y_pred, output_dict = True)
        real_res.append(cr['accuracy'])
        print(f"--------------------------------TSF acc is {cr['accuracy']}-------------------------------")
        ...
        cr = classification_report(test_y, res, output_dict = True)
        real_res.append(cr['accuracy'])
        print(f"------------------------rck shuffle acc is {cr['accuracy']}--------------------------------------")
        ...
        cr = classification_report(test_y, y_pred, output_dict = True)
        real_res.append(cr['accuracy'])
        print(f"--------------------------------TSF shuffle acc is {cr['accuracy']}-------------------------------")
