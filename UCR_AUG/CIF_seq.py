...
for dataset_name in equalLengthProblems:
    ...
    # train_x, train_y = load_from_tsfile(f"/data/z5191634/data/Univariate_ts/{dataset_name}/{dataset_name}_TRAIN.ts", return_data_type="numpy2d")
    # test_x, test_y = load_from_tsfile(f"/data/z5191634/data/Univariate_ts/{dataset_name}/{dataset_name}_TEST.ts", return_data_type="numpy2d")
...
my_log = logger(["Minirocket", "Minirocket_AUG"], equalLengthProblems, f"CIF_seq{padding}")
for dataset in range(len(equalLengthProblems)):
    ...
    random.seed(0)
    np.random.seed(0)
    clf = CanonicalIntervalForest(n_jobs=20) 
